...
# Compute VIF for each feature in your X matrix
vif_df = pd.DataFrame()
vif_df['feature'] = X.columns
vif_df['VIF'] = [variance_inflation_factor(X.values, i) for i in range(X.shape[1])]
print(vif_df.sort_values('VIF', ascending=False))

# Iteratively drop the feature with highest VIF until all VIFs are below 5
while True:
    ...
    print(f"Dropping {X.columns[drop_idx]} (VIF={max_vif:.2f})")
    X = X.drop(columns=[X.columns[drop_idx]])
...
# Iteratively drop highest VIF until all < 5
while compute_vif(X)['VIF'].max() > 5:
    drop_feat = compute_vif(X).iloc[0]['feature']
    print(f'Dropping {drop_feat}')
...
Model too complex — fits noise as if it were signal
Symptom on train
High error
Very low error
Symptom on test
High error (similar to train)
Much higher error than train
Cause
Too few features, low-degree polynomial, too much regularization
Too many features, high-degree polynomial, no regularization
Fix
Add features, increase model complexity, reduce regularization
Reduce features, regularize (Ridge/Lasso), add data, cross-validate
...
Reduce by
Bias
How far off the model's average prediction is from truth
Model too simple — assumptions don't fit reality
More features, complex model, less regularization
Variance
How much predictions jump around for different training sets
Model too complex — too sensitive to specific training data
Fewer features, regularization, more training data
Irreducible
Random noise inherent in y
Real-world randomness, measurement error
Cannot be removed
...
Ridge (L2)
α · Σβⱼ²
Shrinks all coefficients toward zero (none exactly zero). Best for many small effects.
Lasso (L1)
α · Σ|βⱼ|
Sets some coefficients exactly to zero → automatic feature selection.
ElasticNet
α₁·Σβⱼ² + α₂·Σ|βⱼ|
Combines L1 and L2. Good when features are correlated.
...
bias_error = 1 - scores.mean()      # operational bias
variance_error = scores.std()       # operational variance

print(f"Bias error:     {bias_error:.4f}")
print(f"Variance error: {variance_error:.4f}")
...
α small (e.g. 0.01)
Mild shrinkage; close to OLS
α moderate (e.g. 1.0)
Substantial shrinkage; strong regularization
α very large (e.g. 1000)
All coefficients ≈ 0 → severe underfitting
...
Impact on coefs
Shrinks all βs toward 0; never exactly 0
Shrinks and sets some βs exactly to 0
Feature selection?
No
Yes (sparse solution)
Multicollinearity
Handles well — distributes effect
Picks one feature, zeros others (arbitrary)
Use case
All features moderately useful; multicollinearity present
Few features actually useful; sparse interpretable model needed
...
# 1. Scale features (essential before regularization)
scaler = StandardScaler()
X_train_scaled = scaler.fit_transform(X_train)
X_test_scaled = scaler.transform(X_test)

# 2. Build all three models with hyperparameters from your paper
ridge = Ridge(alpha=1, max_iter=500)            # exam-spec for Aug 2023, Mar 2024
lasso = Lasso(alpha=0.01, max_iter=500)         # exam-spec everywhere
elastic = ElasticNet(alpha=0.1, l1_ratio=0.5)   # Feb 2025 spec
...
for name, model in [('Ridge', ridge), ('Lasso', lasso), ('ElasticNet', elastic)]:
    ...
    n_zero = sum(model.coef_ == 0)
    print(f"{name}: Test RMSE={rmse:.2f}, zero coefs={n_zero}")

from sklearn.model_selection import GridSearchCV
from sklearn.linear_model import Ridge

param_grid = {'alpha': [0.001, 0.01, 0.1, 1, 10, 100]}
grid = GridSearchCV(Ridge(max_iter=500), param_grid, cv=5,
...
Combination
Diagnosis
Low bias error, low variance error
Excellent — well-fit, stable model
High bias error, low variance error
Underfitting — model is consistent but bad
Low bias error, high variance error
Overfitting — performance depends on which fold
High bias error, high variance error
Bad model — both poor and unstable
Other CV variants
Variant
What it does
When to use
...
Very small datasets; extremely expensive
Time-Series Split
Train always precedes test in time
Time-series data — never shuffle the time axis
Shuffle Split
Random repeated train/val splits
When you want more iterations than k allows
Exam questions on this topic
1b 1b
...
# Quick way: cross_val_score
model = LinearRegression()
r2_scores = cross_val_score(model, X_train, y_train, cv=5, scoring='r2')

print(f"R² per fold: {r2_scores}")
print(f"Mean R²:  {r2_scores.mean():.4f}")
print(f"Bias error (1 - mean R²):  {1 - r2_scores.mean():.4f}")
print(f"Variance error (std R²):    {r2_scores.std():.4f}")

# If you want RMSE per fold instead of R²
neg_mse = cross_val_score(model, X_train, y_train, cv=5,
...
Meaning
SS_res
Residual sum of squares — how badly the model fits
SS_tot
Total variance in y — how spread out y is around its mean
R²
Fraction of variance the model explains. Closer to 1 = better.
R² value
Interpretation
R² = 1
Model explains 100% of variance — perfect predictions
R² = 0
Model is no better than predicting the mean of y
R² < 0
Model is worse than predicting the mean (yes, possible on test set)
R² ≈ 0.7
Decent — explains ~70% of variance; common in real data
The trap with R²
...
Same as y
Yes (squared)
You care about big errors
MSE
[0, ∞)
y²
Yes (squared)
Internal optimization (training)
MAE
[0, ∞)
Same as y
Less
...
R²
Adjusted R²
Behaviour when adding any feature
Always ↑ or stays same
↑ only if feature genuinely helps; else ↓
Range
(−∞, 1]
Can be lower than R²
...
mape = mean_absolute_percentage_error(y_test, y_pred) * 100  # sklearn returns fraction

# Adjusted R² — sklearn doesn't have it, but it's a one-liner
n = len(y_test)
k = X_test.shape[1]
adj_r2 = 1 - (1 - r2) * (n - 1) / (n - k - 1)

print(f"R²:    {r2:.4f}")
print(f"AdjR²: {adj_r2:.4f}")
...
Normality of residuals
Jarque-Bera
Residuals are normal (skew & kurt = normal)
p < 0.05 → not normal
Homoscedasticity
Breusch-Pagan
...
Variance is constant
p < 0.05 → heteroscedastic
Independence (autocorr.)
Durbin-Watson
No autocorrelation
Statistic far from 2.0
Linearity
Rainbow test
...
# Suppose you've already fit: ols_model = sm.OLS(y_train, X_const).fit()
...
print(f"Breusch-Pagan homoscedast.: p={bp_p:.4f}")
print(f"Durbin-Watson autocorr.:   {dw:.3f}")
